class Prota {
  ...
  void mover() {
    if (quieto == true) {
      image(img, x-30, y-30, ancho, alto);
    }
    if (keyPressed) {
      if (key =='w') {
        quieto = false;
    ...
    if (keyPressed) {
      if (key =='a') {
        izquierda = true;
        x-= t;
        image (izq, x-30, y-30, ancho, alto);
        quieto = false;
      }
    } else {
      quieto =true;
    ...
    if (keyPressed) {
      if (key =='d') {
        derecha = true;
        x+= t;
        image (der, x-30, y-30, ancho, alto);
        quieto = false;
      } else {
        quieto =true;
      }
      if (keyPressed) {
        if (key =='s') {
          abajo = true;
          y+= t;
          image (down, x-30, y-30, ancho, alto);
          quieto = false;
        } else {
          quieto =true;
        }
